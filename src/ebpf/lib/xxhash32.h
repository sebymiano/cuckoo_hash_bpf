...
static __always_inline uint32_t rotateLeft(const uint32_t x, const unsigned char bits) {
    return (x << bits) | (x >> (32 - bits));
}
/// process a block of 4x4 bytes, this is the main part of the XXHash32 algorithm
static __always_inline void process(const void *data, uint32_t *state0, uint32_t *state1,
...
static __always_inline uint32_t xxhash32_anylength(const void *input, const uint32_t totalLength,
                                                   const uint32_t seed) {
    ...
    // hasher.add(...) call
    {
        ...
        for (unsigned int i = 0; i < bufferSize; i++) {
            buffer[i] = data[i];
            // __bpf_memcpy(buffer + i, data + i, 1);
        }
...
static __always_inline uint32_t xxhash32_short(const void *input, const uint32_t totalLength,
                                               const uint32_t seed) {
    // internal state and temporary buffer
    // uint32_t   state[4]; // state[2] == seed if totalLength < MaxBufferSize
...
static __always_inline uint32_t xxhash32(const void *input, const uint32_t totalLength,
                                         const uint32_t seed) {
