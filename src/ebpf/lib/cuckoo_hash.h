...
static int _name##_cuckoo_insert_loop(uint32_t index, void *data) { \
  struct _name##_cuckoo_insert_loop_ctx *ctx = (struct _name##_cuckoo_insert_loop_ctx *)data; \
...
\
static __always_inline bool _name##_cuckoo_insert(struct _name##_cuckoo_hash_map* map, _name##_cuckoo_key_t *key, _name##_cuckoo_val_t *val) {  \
  struct _name##_cuckoo_hash_cell *elem; \
...
\
static __always_inline _name##_cuckoo_val_t* _name##_cuckoo_lookup(struct _name##_cuckoo_hash_map* map, const _name##_cuckoo_key_t* key) { \
  struct _name##_cuckoo_hash_cell *elem; \
  uint32_t hash; \
\
...
\
static __always_inline bool _name##_cuckoo_delete(struct _name##_cuckoo_hash_map* map, const _name##_cuckoo_key_t* key) { \
  struct _name##_cuckoo_hash_cell *elem; \
  uint32_t hash; \
\
  hash = HASH_TYPE((void *)key, sizeof(_name##_cuckoo_key_t), HASH_SEED_1); \
